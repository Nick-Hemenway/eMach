...
class IMArchitectType1(Architect):
    ...
    def __init__(self, specification):
        ...
        self.__winding = winding_layout_v2(
            DPNV_or_SEPA=True, Qs=24, p=3, ps=4, coil_pitch_y=3
        )
    ...
    def create_new_design(self, x):
        ...
        im_parameters = {
            ...
            "Length_HeadNeckRotorSlot": free_variables[
                "Length_HeadNeckRotorSlot"
            ],  # Jiahao is using 1 mm as default
            ...
            "Width_StatorTeethHeadThickness": free_variables[
            ...
            "layer_phases": [
                self.__winding.layer_X_phases,
                self.__winding.layer_Y_phases,
            ],
            "layer_polarity": [
                self.__winding.layer_X_signs,
                self.__winding.layer_Y_signs,
            ],
    ...
    def __get_current_coil(self):
        stator_phase_current_rms = self.__design_spec["rated_power"] / (
            self.__design_spec["m"] * 0.96 * self.__design_spec["voltage_rating"] * 0.9
        )
    ...
    def __get_pole_pitch_tau_p(self, free_variables):
        air_gap_diameter_D = (
            free_variables["Length_AirGap"] + 2 * free_variables["Radius_OuterRotor"]
        )
    ...
    def __get_turns(self, free_variables):
        ...
        desired_emf_Em = (
            0.95 * self.__design_spec["voltage_rating"]
        )  # 0.96~0.98, high speed motor has higher leakage reactance hence 0.95

        alpha_i = (
            2 / 3.14
        )  # ideal sinusoidal flux density distribusion, when the saturation happens in teeth, alpha_i becomes higher.
        air_gap_flux_Phi_m = (
            alpha_i
            * self.__design_spec["guess_air_gap_flux_density"]
            * self.__get_pole_pitch_tau_p(free_variables)
            * self.__get_l_st
        )
        ExcitationFreqSimulated = self.__design_spec["ExcitationFreqSimulated"]
        no_series_coil_turns_N = np.round(
            np.sqrt(2)
            * desired_emf_Em
            / (
                2
                * np.pi
                * ExcitationFreqSimulated
                * self.__design_spec["kw1"]
                * air_gap_flux_Phi_m
            )
        )
        ...
        no_conductors_per_slot_zQ = (
            2
            * 3
            * no_series_coil_turns_N
            / self.__design_spec["Qs"]
