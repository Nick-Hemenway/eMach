...
class BSPM_EM_Analysis:
    ...
    def analyze(self, problem):
        ...
        expected_project_file = (
            self.configuration["run_folder"] + "%s.jproj" % self.project_name
        )
        ...
        self.design_results_folder = (
            self.configuration["run_folder"] + "%sresults/" % self.project_name
        )
        ...
        study = self.add_magnetic_transient_study(
            app, model, self.configuration["JMAG_csv_folder"], self.study_name
        ...
        if not self.configuration["delete_results_after_calculation"]:
            ...
            app.GetDataManager().GetGraphModel("Circuit Voltage").WriteTable(
                self.configuration["JMAG_csv_folder"]
                + self.study_name
    ...
    def copper_loss(self):
        return (
            ...
            * ((self.current_trms) ** 2 + self.current_srms**2)
            * self.R_coil
        )
    ...
    def pre_process(self, app, model):
        ...
        if len(part_ID_list) != int(
            1 + self.machine_variant.p * 2 + 1 + self.machine_variant.Q * 2
        ):
    ...
    def add_magnetic_transient_study(
        ...
        study.GetStudyProperties().SetValue(
            "NonlinearMaxIteration", self.configuration["max_nonlinear_iterations"]
        )
        study.GetStudyProperties().SetValue(
        ...
        study.GetStudyProperties().SetValue(
            "OnlyTableResults", self.configuration["OnlyTableResults"]
        )
        ...
        if True:
            ...
            number_of_steps_1TS = (
                self.configuration["number_of_steps_per_rev_1TS"]
                * number_of_revolution_1TS
            )
            number_of_steps_2TS = (
                self.configuration["number_of_steps_per_rev_2TS"]
                * number_of_revolution_2TS
            )
            ...
            refarray[2][
                1
            ] = number_of_steps_2TS  # 最后的number_of_steps_2TS（32）步，必须对应半个周期，从而和后面的铁耗计算相对应。
            refarray[2][2] = 50
            ...
            study.GetStep().SetTableProperty(
                "Division", DM.GetDataSet("SectionStepTable")
            )
        ...
        study.GetStudyProperties().SetValue(
            "CsvResultTypes", self.configuration["Csv_Results"]
        )
        study.GetStudyProperties().SetValue(
            "DeleteResultFiles", self.configuration["delete_results_after_calculation"]
        )
    ...
    def add_mesh(self, study, model):
        ...
        study.GetMeshControl().SetValue(
            "RadialDivision", self.configuration["mesh_radial_division"]
        )  # for air region near which motion occurs
        study.GetMeshControl().SetValue(
            "CircumferentialDivision", self.configuration["mesh_circum_division"]
        )  # 1440) # for air region near which motion occurs 这个数足够大，sliding mesh才准确。
        study.GetMeshControl().SetValue(
            "AirRegionScale", self.configuration["mesh_air_region_scale"]
        )  # [Model Length]: Specify a value within (1.05 <= value < 1000)
        study.GetMeshControl().SetValue(
            "MeshSize", self.configuration["mesh_size"] * 1e3
        )  # mm
        study.GetMeshControl().SetValue("AutoAirMeshSize", 0)
        study.GetMeshControl().SetValue(
            "AirMeshSize", self.configuration["mesh_size"] * 1e3
        )  # mm
        ...
        study.GetMeshControl().GetCondition("MagnetMeshCtrl").SetValue(
            "Size", self.configuration["mesh_magnet_size"] * 1e3
        )  # mm
    ...
    def create_custom_material(self, app, steel_name):
        ...
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("ShearModulus", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("YoungModulusX", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("YoungModulusY", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("YoungModulusZ", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("ShearModulusXY", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("ShearModulusYZ", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("ShearModulusZX", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("MagnetizationSaturated2", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
        ).SetValue("MagnetizationSaturatedMakerValue", 0)
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.stator_iron_mat["core_material"]
    ...
    def add_circuit(self, app, model, study, bool_3PhaseCurrentSource=True):
        ...
        def circuit(poles, turns, Rs, ampT, ampS, freq, x=10, y=10):
            ...
            study.GetCircuit().CreateTerminalLabel(
                "Terminal_U", x - 4, -1 * (y + y_offset + 2)
            )
            study.GetCircuit().CreateTerminalLabel(
                "Terminal_V", x + 4, -1 * (y + y_offset + 2)
            )
            study.GetCircuit().CreateTerminalLabel(
                "Terminal_W", x + 12, -1 * (y + y_offset + 2)
            )
            study.GetCircuit().CreateTerminalLabel(
                "Terminal_X", x + 20, -1 * (y + y_offset + 2)
            )
            study.GetCircuit().CreateTerminalLabel(
                "Terminal_Y", x + 28, -1 * (y + y_offset + 2)
            )

        current_tpeak = self.current_trms * np.sqrt(
            2
        )  # It, max current at torque terminal
        current_speak = self.current_srms * np.sqrt(
            2
        )  # Is, max current at suspension terminal Is+It/2

        slot_area_utilizing_ratio = (current_tpeak + current_speak) / (
            self.machine_variant.Rated_current * np.sqrt(2)
        )
        print("---Slot area utilizing ratio is", slot_area_utilizing_ratio)
        print(
            "---Peak Current per coil :",
            self.machine_variant.Rated_current * np.sqrt(2),
        )
    ...
    def extract_JMAG_results(self, path, study_name):
        ...
        range_2TS = int(
            self.configuration["number_of_steps_per_rev_2TS"]
            * self.configuration["number_of_revolution_2TS"]
        )
