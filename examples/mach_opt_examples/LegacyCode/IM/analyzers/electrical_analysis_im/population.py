...
class swarm(object):
    ...
    def draw_jmag_model(self, individual_index, im_variant, model_name, bool_trimDrawer_or_vanGogh=True, doNotRotateCopy=False):
        ...
        try:
            if bool_trimDrawer_or_vanGogh:
                ...
                print("Call Vangogh at population")
    ...
    def run(self, im, individual_index=0, run_list=[1,1,0,0,0]):
        ...
        if model.NumStudies()<3:
            ...
            # IEMDC
            number_cycles_prolonged = self.fea_config_dict['designer.number_cycles_prolonged'] # 150 or 1
            ...
            if number_cycles_prolonged == 0: # 不延长了，就是普通的Tran2TSS
                ...
                refarray[2][2] =        50
                number_of_total_steps = 1 + number_of_steps_1stTSS + number_of_steps_2ndTSS # [Double Check] don't forget to modify here!
            else:
                ...
                refarray[4][1] =    400
                refarray[4][2] =        50
                number_of_total_steps = 1 + number_of_steps_1stTSS + number_of_steps_2ndTSS + number_cycles_prolonged*self.fea_config_dict['designer.TranRef-StepPerCycle'] + 400 # [Double Check] don't forget to modify here!
    ...
    def show_results_iemdc19(self, im_variant, femm_solver_data=None, femm_rotor_current_function=None):
        ...
        if int(self.im.Qr) == 36:
            pass
            # fig_cur2.savefig(r'./Qr36_rotor_current_1000A.pdf', dpi=400, bbox_inches='tight')
            pass
            # fig_main.savefig(r'./FEA_Model_Comparisons.pdf', dpi=400, bbox_inches='tight')
    ...
    def read_csv_results_4_optimization(self, study_name, path_prefix=None):
        ...
        effective_part = rotor_copper_loss_list[:int(0.5*self.fea_config_dict['designer.number_of_steps_2ndTSS'])] # number_of_steps_2ndTSS = steps for half peirod
    ...
    def run_study(self, im_variant, app, study, toc):
        ...
        if self.fea_config_dict['designer.JMAG_Scheduler'] == False:
            ...
            print('Submit to JMAG Scheduler...')
            job = study.CreateJob()
...
class bearingless_induction_motor_design(object):
    ...
    def __init__(self, spec_input_dict=None, spec_derive_dict=None, spec_geometry_dict=None, fea_config_dict=None, model_name_prefix='PS'):
        ...
        self.spec_derive_dict = spec_derive_dict
        self.spec_geometry_dict = spec_geometry_dict
        self.fea_config_dict = fea_config_dict
        ...
        #02 Pyrhonen Data
        if spec_geometry_dict != None:
            self.Qs = spec_geometry_dict['Qs']
            self.Qr = spec_geometry_dict['Qr']
            self.Angle_StatorSlotSpan = 360. / self.Qs # in deg.
            self.Angle_RotorSlotSpan  = 360. / self.Qr # in deg.

            self.Radius_OuterStatorYoke = spec_geometry_dict['Radius_OuterStatorYoke']
            self.Radius_InnerStatorYoke = spec_geometry_dict['Radius_InnerStatorYoke']
            self.Length_AirGap          = spec_geometry_dict['Length_AirGap']
            self.Radius_OuterRotor      = spec_geometry_dict['Radius_OuterRotor']
            self.Radius_Shaft           = spec_geometry_dict['Radius_Shaft']

            self.Length_HeadNeckRotorSlot = spec_geometry_dict['Length_HeadNeckRotorSlot']
            self.Radius_of_RotorSlot      = spec_geometry_dict['Radius_of_RotorSlot']
            self.Location_RotorBarCenter  = spec_geometry_dict['Location_RotorBarCenter']
            self.Width_RotorSlotOpen      = spec_geometry_dict['Width_RotorSlotOpen']

            self.Radius_of_RotorSlot2     = spec_geometry_dict['Radius_of_RotorSlot2']
            self.Location_RotorBarCenter2 = spec_geometry_dict['Location_RotorBarCenter2']

            self.Angle_StatorSlotOpen            = spec_geometry_dict['Angle_StatorSlotOpen']
            self.Width_StatorTeethBody           = spec_geometry_dict['Width_StatorTeethBody']
            ...
            self.BeariW_poles       = spec_geometry_dict['BeariW_Poles']

            self.ID = self.name + '-' + self.model_name_prefix #+ '-Qr%d'%(self.Qr)
        else:
        ...
        # else:
        self.use_drop_shape_rotor_bar = spec_input_dict['use_drop_shape_rotor_bar'] # Since 5/23/2019
        ...
        if True:
            self.x_denorm_names = [None]*9
            self.x_denorm_names[0] = 'Air Gap Length: delta'
            self.x_denorm_names[1] = 'Stator Tooth Width: w_st'
            self.x_denorm_names[2] = 'Rotor Tooth Width: w_rt'
            self.x_denorm_names[3] = 'Stator Slot Open Angle: theta_so' # need to be conveted to alpha_st
            self.x_denorm_names[4] = 'Rotor Slot Open Width: w_ro' # need to be converted to alpha_rt
            self.x_denorm_names[5] = 'Stator Tooth Tip Depth: d_so'
            self.x_denorm_names[6] = 'Rotor Tooth Tip Depth: d_ro'
            self.x_denorm_names[7] = 'Stator Slot Depth: d_st'
            self.x_denorm_names[8] = 'Stator Yoke Depth: d_sy'
    ...
    def build_x_denorm(self):
        ...
        #             spec.d_so,
        #             spec.d_ro]
        # if fea_config_dict['which_filter'] == 'VariableStatorSlotDepth':
        #     x_denorm += [spec.d_st]
        ...
        if 'VariableStatorSlotDepth' in self.fea_config_dict['which_filter']:
            ...
            x_denorm[6] = self.spec_geometry_dict['Length_HeadNeckRotorSlot']

            # Constranint #2
            # stator_tooth_width_b_ds imposes constraint on stator slot height
            stator_tooth_width_b_ds = self.spec_geometry_dict['Width_StatorTeethBody']*1e-3
            Width_StatorTeethNeck = 0.5 * self.spec_geometry_dict['Width_StatorTeethHeadThickness']

            area_stator_slot_Sus    = self.spec_geometry_dict['area_stator_slot_Sus']
            air_gap_length_delta    = self.spec_geometry_dict['Length_AirGap']*1e-3
            stator_inner_radius_r_is = self.Radius_OuterRotor*1e-3 + air_gap_length_delta 
            stator_yoke_diameter_Dsyi = self.get_stator_yoke_diameter_Dsyi(  stator_tooth_width_b_ds, 
                                                                        ...
                                                                        self.Qs,
                                                                        self.spec_geometry_dict['Width_StatorTeethHeadThickness'],
                                                                        Width_StatorTeethNeck)

            stator_tooth_height_h_ds = 0.5*(stator_yoke_diameter_Dsyi - 2*stator_inner_radius_r_is)
            x_denorm[7] = stator_tooth_height_h_ds*1e3

            if 'VariableStatorSlotDepth_VariableStatorYokeDepth' in self.fea_config_dict['which_filter']:
                My_Radius_OuterStatorYoke = self.spec_geometry_dict['Radius_OuterStatorYoke'] # see method local_design_variant()
                stator_yoke_height_h_ys = 0.5*(My_Radius_OuterStatorYoke*1e-3 - stator_yoke_diameter_Dsyi)
                if stator_yoke_height_h_ys < 0:
                    print('stator_yoke_diameter_Dsyi is', stator_yoke_diameter_Dsyi)
                    print('My_Radius_OuterStatorYoke is', My_Radius_OuterStatorYoke)
                    print('Negative stator_yoke_height_h_ys is detected. This is bug to be fixed...')
                    # raise Exception('Negative stator_yoke_height_h_ys is detected...')
                    stator_yoke_height_h_ys = abs(stator_yoke_height_h_ys)
                x_denorm += [None]
                x_denorm[8] = stator_yoke_height_h_ys*1e3

        else:
            x_denorm = [None]*7
            x_denorm[0] = self.spec_geometry_dict['Length_AirGap']
            x_denorm[1] = self.spec_geometry_dict['Width_StatorTeethBody']
            x_denorm[2] = self.spec_geometry_dict['rotor_tooth_width_b_dr']*1e3 # Radius_of_RotorSlot = 1e3 * (2*pi*(im.Radius_OuterRotor - Length_HeadNeckRotorSlot)*1e-3 - rotor_tooth_width_b_dr*im.Qr) / (2*im.Qr+2*pi)
            x_denorm[3] = self.spec_geometry_dict['Angle_StatorSlotOpen']
            x_denorm[4] = self.spec_geometry_dict['Width_RotorSlotOpen']
            x_denorm[5] = self.spec_geometry_dict['Width_StatorTeethHeadThickness']
            x_denorm[6] = self.spec_geometry_dict['Length_HeadNeckRotorSlot']
    ...
    def local_design_variant(cls, im, number_current_generation, individual_index, x_denorm):
        ...
        area_stator_slot_Sus = im.spec_geometry_dict['area_stator_slot_Sus']
        ...
        if 'VariableStatorSlotDepth' in im.fea_config_dict['which_filter']:
            ...
            stator_yoke_diameter_Dsyi = 2*stator_inner_radius_r_is + 2*stator_tooth_height_h_ds
            if 'VariableStatorSlotDepth_VariableStatorYokeDepth' in im.fea_config_dict['which_filter']:
                # stator yoke depth depends on yoke depth
                stator_yoke_height_h_ys = x_denorm[8]*1e-3
                My_Radius_OuterStatorYoke = 0.5*(stator_yoke_diameter_Dsyi + 2*stator_yoke_height_h_ys) * 1e3
            else:
                # stator yoke depth is fixed.
        ...
        rotor_tooth_height_h_dr, rotor_tooth_width_b_dr, thermal_penalty, new__area_rotor_slot_Sur = cls.get_rotor_tooth_height_h_dr(  rotor_tooth_width_b_dr,
                                                                ...
                                                                im.Qr,
                                                                Length_HeadNeckRotorSlot,
                                                                im.spec_geometry_dict['minimum__area_rotor_slot_Sur'])
        rotor_slot_height_h_sr = rotor_tooth_height_h_dr


        # radius of outer rotor slot
        Radius_of_RotorSlot = 1e3 * (2*pi*(im.Radius_OuterRotor - Length_HeadNeckRotorSlot)*1e-3 - rotor_tooth_width_b_dr*im.Qr) / (2*im.Qr+2*pi) # see radius_rotor_slot_formula.wmf
        Location_RotorBarCenter = im.Radius_OuterRotor - Length_HeadNeckRotorSlot - Radius_of_RotorSlot

        # debug
        # print(':::', im.Radius_OuterRotor, Length_HeadNeckRotorSlot, rotor_tooth_width_b_dr, Radius_of_RotorSlot)
        # quit()
        ...
        if Width_RotorSlotOpen>2*Radius_of_RotorSlot:
            logger = logging.getLogger(__name__)
            msg = 'Constraint #1: Rotor slot opening cannot be larger than rotor slot width. Gen#%04d. Individual index=%d.' % (number_current_generation, individual_index)
            print(msg)
            logger.warn(msg)
        ...
        new_spec_geometry_dict = dict()
        new_spec_geometry_dict['Qs'] = im.Qs
        new_spec_geometry_dict['Qr'] = im.Qr
        new_spec_geometry_dict['Radius_OuterStatorYoke']         = My_Radius_OuterStatorYoke #im.Radius_OuterStatorYoke
        print('[DEBUG]: Radius_OuterStatorYoke is', new_spec_geometry_dict['Radius_OuterStatorYoke'], 'while the origianl is', im.Radius_OuterStatorYoke)
        new_spec_geometry_dict['Radius_InnerStatorYoke']         = 0.5*stator_yoke_diameter_Dsyi * 1e3 # 定子内轭部处的半径由需要的定子槽面积和定子齿宽来决定。
        new_spec_geometry_dict['Length_AirGap']                  = x_denorm[0]                # [1] # Length_AirGap
        new_spec_geometry_dict['Radius_OuterRotor']              = im.Radius_OuterRotor
        new_spec_geometry_dict['Radius_Shaft']                   = im.Radius_Shaft 
        new_spec_geometry_dict['Length_HeadNeckRotorSlot']       = Length_HeadNeckRotorSlot            # [6]
        new_spec_geometry_dict['Radius_of_RotorSlot']            = Radius_of_RotorSlot                 # inferred from [2]
        new_spec_geometry_dict['Location_RotorBarCenter']        = Location_RotorBarCenter 
        new_spec_geometry_dict['Width_RotorSlotOpen']            = Width_RotorSlotOpen                 # [4]
        new_spec_geometry_dict['Radius_of_RotorSlot2']           = Radius_of_RotorSlot2
        new_spec_geometry_dict['Location_RotorBarCenter2']       = Location_RotorBarCenter2
        new_spec_geometry_dict['Angle_StatorSlotOpen']           = x_denorm[3]                # [3] # Angle_StatorSlotOpen
        new_spec_geometry_dict['Width_StatorTeethBody']          = x_denorm[1]                # [1] # Width_StatorTeethBody
        new_spec_geometry_dict['Width_StatorTeethHeadThickness'] = Width_StatorTeethHeadThickness      # [5]
        new_spec_geometry_dict['Width_StatorTeethNeck']          = Width_StatorTeethNeck

        new_spec_geometry_dict['rotor_tooth_width_b_dr']         = rotor_tooth_width_b_dr

        new_spec_geometry_dict['DriveW_poles']                   = im.DriveW_poles 
        new_spec_geometry_dict['DriveW_zQ']                      = im.DriveW_zQ
        new_spec_geometry_dict['DriveW_Rs']                      = im.DriveW_Rs    
        new_spec_geometry_dict['DriveW_CurrentAmp']              = im.spec_geometry_dict['DriveW_CurrentAmp'] #
        new_spec_geometry_dict['DriveW_Freq']                    = im.DriveW_Freq
        new_spec_geometry_dict['stack_length']                   = im.stack_length
        new_spec_geometry_dict['area_stator_slot_Sus']           = area_stator_slot_Sus
        new_spec_geometry_dict['area_rotor_slot_Sur']            = area_rotor_slot_Sur
        new_spec_geometry_dict['minimum__area_rotor_slot_Sur']   = im.spec_geometry_dict['minimum__area_rotor_slot_Sur']
        
        new_spec_geometry_dict['rotor_tooth_flux_density_B_dr']  = im.spec_geometry_dict['rotor_tooth_flux_density_B_dr']
        new_spec_geometry_dict['stator_tooth_flux_density_B_ds'] = im.spec_geometry_dict['stator_tooth_flux_density_B_ds']
        new_spec_geometry_dict['Jr']                             = im.spec_geometry_dict['Jr']
        new_spec_geometry_dict['BeariW_Poles']                   = im.spec_geometry_dict['BeariW_Poles']

        # initialze with the class's __init__ method
        self = cls( im.spec_input_dict,
                    im.spec_derive_dict,
    ...
    def reproduce_the_problematic_design(cls, path_to_shitty_design_file):
        ...
        with open(path_to_shitty_design_file, 'r') as f:
            ...
            while True:
                ...
                # take case of object value
                if 'winding_layout' in last_line:
                    break
    ...
    def pre_process(self, app):
        ...
        self.id_shaft = part_ID_list[0]
        self.id_rotorCore = part_ID_list[1]
        self.partIDRange_Cage = part_ID_list[2 : 2+int(self.Qr)]
        self.id_statorCore    = part_ID_list[2+int(self.Qr)]
        self.partIDRange_Coil = part_ID_list[3+int(self.Qr) : 3+int(self.Qr) + int(self.Qs*2)]
        ...
        group("Cage", self.partIDRange_Cage) # 59-44 = 15 = self.Qr - 1
        group("Coil", self.partIDRange_Coil) # 107-60 = 47 = 48-1 = self.Qs*2 - 1
        # group(u"AirWithinRotorSlots", partIDRange_AirWithinRotorSlots) # 123-108 = 15 = self.Qr - 1


        ''' Add Part to Set for later references '''
        def part_set(name, x, y, id=None):
            model.GetSetList().CreatePartSet(name)
            model.GetSetList().GetSet(name).SetMatcherType("Selection")
            model.GetSetList().GetSet(name).ClearParts()
            sel = model.GetSetList().GetSet(name).GetSelection()
            # print x,y
            if id is None:
                sel.SelectPartByPosition(x,y,0.0) # z=0 for 2D
            else:
                sel.SelectPart(id)
            model.GetSetList().GetSet(name).AddSelected(sel)
        ...
        if bool_add_part_to_set_by_id == True:
            part_set("ShaftSet", None, None, id=self.id_shaft)
        else:
            part_set("ShaftSet", 0.0, 0.0)

        # Create Set for drive poles Winding (i.e., X layer or right layer)
        ...
        THETA = pi + (0.5*(self.Angle_StatorSlotSpan) -  0.05*(self.Angle_StatorSlotSpan-self.Width_StatorTeethBody))/180.*pi
        X = R*cos(THETA)
        Y = R*sin(THETA)
        count = 0
        for UVW, UpDown in zip(self.wily.layer_X_phases, self.wily.layer_X_signs):
            count += 1 
            # print('LX', THETA/pi*180)
            part_set("CoilLX%s%s %d"%(UVW,UpDown,count), X, Y)

            THETA += self.Angle_StatorSlotSpan/180.*pi
            X = R*cos(THETA)
            Y = R*sin(THETA)

        # Create Set for bearing poles Winding (i.e., Y layer or left layer)
            # THETA = (0.5*(self.Angle_StatorSlotSpan) +  0.05*(self.Angle_StatorSlotSpan-self.Angle_StatorSlotOpen))/180.*pi
        THETA = pi + (0.5*(self.Angle_StatorSlotSpan) +  0.05*(self.Angle_StatorSlotSpan-self.Width_StatorTeethBody))/180.*pi
        X = R*cos(THETA)
        Y = R*sin(THETA)
        ...
        for UVW, UpDown in zip(self.wily.layer_Y_phases, self.wily.layer_Y_signs):
            count += 1 
            # print('LY', THETA/pi*180)
            part_set("CoilLY%s%s %d"%(UVW,UpDown,count), X, Y)
            THETA += self.Angle_StatorSlotSpan/180.*pi
            X = R*cos(THETA)
            Y = R*sin(THETA)
        ...
        # Create Set for Bars
        list_xy_bars = []
        if bool_add_part_to_set_by_id == True: # Option 1: add part to set by its ID.
            for index, id in enumerate(self.partIDRange_Cage):
                natural_index = index + 1
                part_set("Bar %d"%(natural_index), None, None, id=id)

        else: # Option 2: set Bar set by location | 有时候会把定子铁芯和Bar 1弄成一个set，不知道为什么
        ...
        # Create Set for Motion Region
        def part_list_set(name, list_xy, list_id, prefix=None):
            model.GetSetList().CreatePartSet(name)
            model.GetSetList().GetSet(name).SetMatcherType("Selection")
            model.GetSetList().GetSet(name).ClearParts()
            sel = model.GetSetList().GetSet(name).GetSelection() 
        ...
        if bool_add_part_to_set_by_id == True:
            # print('|||||||', type(self.partIDRange_Cage), self.partIDRange_Cage)
            part_list_set('Motion_Region', [], [self.id_rotorCore, self.id_shaft]+list(self.partIDRange_Cage))
        else:
            part_list_set('Motion_Region', [[0.0,0.0],[0.0,self.Radius_Shaft+EPS]] + list_xy_bars)
    ...
    def add_circuit(self, app, model, study, bool_3PhaseCurrentSource=True):
        ...
        def circuit(Grouping,turns,Rs,ampD,ampB,freq,phase=0, CommutatingSequenceD=0, CommutatingSequenceB=0, x=10,y=10+JMAG_CIRCUIT_Y_POSITION_BIAS_FOR_CURRENT_SOURCE, bool_3PhaseCurrentSource=True):
            ...
            if bool_3PhaseCurrentSource == True: # must use this for frequency analysis
                ...
                if Grouping == 'Torque':
                    ...
                    study.GetCircuit().GetComponent("CS%s"%(Grouping)).SetValue("CommutatingSequence", JMAGCommutatingSequence)
        ...
        circuit(Function,  self.BeariW_turns/nwl, bool_3PhaseCurrentSource=bool_3PhaseCurrentSource,
        ...
        # if self.fea_config_dict['DPNV_separate_winding_implementation'] == True or self.spec_input_dict['DPNV_or_SEPA'] == False:
        if self.spec_input_dict['DPNV_or_SEPA'] == False: # Separate Winding
            ...
            for UVW, UpDown in zip(self.wily.layer_X_phases, self.wily.layer_X_signs):
                ...
                subcondition.ClearParts()
                subcondition.AddSet(model.GetSetList().GetSet("CoilLX%s%s %d"%(UVW,UpDown,count)), 0) # poles=4 means right layer, rather than actual poles
                subcondition.SetValue("Direction2D", dict_dir[UpDown])
                ...
                if self.DriveW_poles == 2:
                    ...
                    for i in range(int(self.no_slot_per_pole)):
                        ...
                        if self.End_Ring_Resistance == 0: # setting a small value to End_Ring_Resistance is a bad idea (slow down the solver). Instead, don't model it
                            ...
                            raise Exception('Not implemented error: pole-specific rotor winding circuit for %d poles are not implemented'%(im.DriveW_poles))
                else:
                    raise Exception('Not implemented error: pole-specific rotor winding circuit for %d poles and non-zero End_Ring_Resistance are not implemented'%(im.DriveW_poles))
...
class TrimDrawer(object):
    ...
    def plot_rotorCore(self, name=None):
        ...
        # print(self.im.Radius_of_RotorSlot)
        # quit()
...
def add_Arnon5(app, dir_parent):
    print('Use Arnon7 instead of Arnon5.')
    print('Use Arnon7 instead of Arnon5.')
    print('Use Arnon7 instead of Arnon5.')
    ...
    # BH = np.loadtxt(dir_parent + 'D:/DrH/Codes/c/Arnon5/Arnon5-final.txt', unpack=True, usecols=(0,1))
